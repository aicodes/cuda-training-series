...
__global__ void vadd(const float *A, const float *B, float *C, int ds){
  ...
  // Notice here that id still starts from threadIdx + blockDim * blockIdx;
  // This is because we still have the full coverage of  gridDim*blockDim threads to do the work.

  // [0 ... blockDim-1] threads (per block), repeated blockDim times.
